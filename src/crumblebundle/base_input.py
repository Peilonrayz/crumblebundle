...
from .input import raw_prompt, KeyCodes
...
PROMPT = '{fg.GREEN}?{fg.RESET} {st.BRIGHT}{fg.YELLOW}{{}}:{st.RESET_ALL} {fg.CYAN}{{}}{st.RESET_ALL}'.format(fg=colorama.Fore, st=colorama.Style)
DEFAULT = '{fg.BLACK}{st.BRIGHT}{{}}{st.RESET_ALL}'.format(fg=colorama.Fore, st=colorama.Style)
OPTION = {
...
MULTI = {
    USAGE: '{fg.RESET}(Press {fg.CYAN}{st.BRIGHT}<space>{st.RESET_ALL}{fg.RESET} to select, {fg.CYAN}{st.BRIGHT}<a>{st.RESET_ALL}{fg.RESET} to toggle all, {fg.CYAN}{st.BRIGHT}<i>{st.RESET_ALL}{fg.RESET} to invert selection){st.RESET_ALL}'.format(fg=colorama.Fore, st=colorama.Style),
    Cursor.NONE: '{fg.RESET} ( ) {{}}{fg.RESET}'.format(fg=colorama.Fore),
    Cursor.HOVER: '{fg.CYAN}>( ) {{}}{fg.RESET}'.format(fg=colorama.Fore),
    Cursor.SELECTED: ' {fg.GREEN}(*){fg.RESET} {{}}{fg.RESET}'.format(fg=colorama.Fore),
    Cursor.SELECTED | Cursor.HOVER: '{fg.CYAN}>{fg.GREEN}(*){fg.CYAN} {{}}{fg.RESET}'.format(fg=colorama.Fore)
}
...
class Input:
    ...
    def value(self, prompt, output, default):
        ...
        default = self.instance.dependency.env.from_string(default).render(cookiecutter=self.instance.cookiecutter)
        value = []
        from_end = 0
        with raw_prompt() as get_char:
            while True:
                print('\r\x1b[K' + PROMPT.format(prompt, ''.join(value) or DEFAULT.format(default)), end='\b' * (from_end if value else len(default)))
                char = get_char()
                if char.raw:
    ...
    def option(self, prompt, output, options, default):
        ...
        with raw_prompt() as get_char:
            ...
            while True:
                ...
                if char is KeyCodes.ENTER:
                    print(clean(len(c_options) + 1) + PROMPT.format(prompt, c_options[index].orig))
                    return c_options[index].orig

                if char is KeyCodes.UP_ARROW or char is KeyCodes.DOWN_ARROW:
                    old_index = index
                    index = (index + (-1 if char == KeyCodes.UP_ARROW else 1)) % len(options)
    ...
    def multiple(self, prompt, output, options, default):
        ...
        with raw_prompt() as get_char:
            ...
            while True:
                ...
                if char is KeyCodes.ENTER:
                    values = tuple(o.orig for o in c_options if o.flags & Cursor.SELECTED)
                    print(clean(len(c_options) + 1) + PROMPT.format(prompt, ', '.join(values)))
                    return values
