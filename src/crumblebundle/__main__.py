...
from .item_values import item_value, multiple, option, value
...
value(
    "package:meta.description", "package.description", "a CrumbleBundle example project"
)
        ...
        "package": {
            "host": {
                ...
                "user_name": input.value(
                    "Github Username", "package.host.user_name", "jhon_doe"
                ),
                "repo_name": input.value(
                    "Github Repo Name",
                    "package.host.repo_name",
                    "{{ cookiecutter.package.name|lower|replace(' ','-') }}",
                ),
            }
        }
    }
...
config = {
    "package": {
        "user": {"name": "Peilonrayz", "email": "pelionrayz@gmail.com"},
        "host": {
